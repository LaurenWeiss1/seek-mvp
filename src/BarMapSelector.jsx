...
const nycNeighborhoods = [
  'SoHo', 'Lower East Side', 'East Village', 'West Village', 'Midtown', 'Chelsea',
  'Hell’s Kitchen', 'Brooklyn - Williamsburg', 'Brooklyn - Bushwick',
  'Brooklyn - Greenpoint', 'Upper East Side', 'Upper West Side'
];
...
function CheckIn() {
  ...
  return (
    <div className="min-h-screen overflow-y-auto pt-20 px-4 bg-gradient-to-br from-[#1f2937] via-[#111827] to-[#1f2937] bg-[length:200%_200%] bg-[position:0%_50%] animate-gradient">
      <form onSubmit={handleSubmit} className="text-white max-w-xl mx-auto p-6 space-y-6">
        ...
        <label className="block text-sm font-medium text-white">Where are you in NYC?</label>
        <select name="city" value={formData.city} onChange={handleChange} className="w-full p-2 rounded bg-white text-black" required>
          <option value="">Select a neighborhood</option>
          {nycNeighborhoods.map(n => <option key={n} value={n}>{n}</option>)}
        </select>
        ...
        {formData.city && (
          <>
            <input
              ...
              className="w-full p-2 mt-2 rounded bg-white text-black"
            />
            ...
            <select
              ...
              className="w-full p-2 rounded bg-white text-black mt-2"
            >
              <option value="">Select a Bar</option>
              {bars.filter(b =>
                b.city.toLowerCase() === formData.city.toLowerCase() &&
                b.name.toLowerCase().includes(searchBar.toLowerCase())
              ).map(b => (
            ...
            <div className="text-sm text-gray-400 text-center mt-1">
              Don’t see your bar?{' '}
              <span className="underline cursor-pointer" onClick={() => setBarNotListed(true)}>
                Add it manually
              </span>
            </div>
          </>
        ...
        {barNotListed && (
          <input
            ...
            className="w-full p-2 rounded bg-[#1F2937] border border-gray-600 mt-2"
          />
        )}
